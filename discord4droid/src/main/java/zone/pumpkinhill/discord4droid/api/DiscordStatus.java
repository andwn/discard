...
import zone.pumpkinhill.discord4droid.json.responses.MetricResponse;
import zone.pumpkinhill.discord4droid.json.responses.StatusResponse;
...
public class DiscordStatus {
    private final static String TAG = DiscordStatus.class.getCanonicalName();
    ...
    public static double getAPIResponseTimeForDay() {
        ...
        try {
            response = DiscordUtils.GSON.fromJson(Requests.GET.makeRequest(
                    String.format(Endpoints.METRICS, "day")), MetricResponse.class);
    ...
    public static double getAPIResponseTimeForWeek() {
        ...
        try {
            response = DiscordUtils.GSON.fromJson(Requests.GET.makeRequest(
                    String.format(Endpoints.METRICS, "week")), MetricResponse.class);
    ...
    public static double getAPIResponseTimeForMonth() {
        ...
        try {
            response = DiscordUtils.GSON.fromJson(Requests.GET.makeRequest(
                    String.format(Endpoints.METRICS, "month")), MetricResponse.class);
        } catch (HTTP429Exception | DiscordException e) {
            Log.e(TAG, "Error getting metrics: " + e);
            return -1;
        }
    ...
    public static Maintenance[] getActiveMaintenances() throws HTTP429Exception, DiscordException {
        StatusResponse response = DiscordUtils.GSON.fromJson(Requests.GET.makeRequest(
                String.format(Endpoints.STATUS, "active")), StatusResponse.class);
    ...
    public static Maintenance[] getUpcomingMaintenances() throws HTTP429Exception, DiscordException {
        StatusResponse response = DiscordUtils.GSON.fromJson(Requests.GET.makeRequest(
                String.format(Endpoints.STATUS, "upcoming")), StatusResponse.class);
        Maintenance[] maintenances = new Maintenance[response.scheduled_maintenances.length];
    ...
    public static class Maintenance {
        ...
        protected Maintenance(String name, String description, String id, Date start, Date stop) {
            this.name = name;
